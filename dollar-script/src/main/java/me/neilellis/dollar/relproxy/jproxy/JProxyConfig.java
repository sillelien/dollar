...
public interface JProxyConfig {
     ...
     *
     * <p>This setting is optional and the folder must be included in Java classpath because the objective is to avoid
     * recompiling.</p>
     *
     * <p>Be careful when executing several Java scripts in the same time and source code has been changed, some file
     * write collisions may happen.</p>
     *
     * @param classFolder the folder where to save .class files. By default is null (not defined, .class files are not
     *                    saved).
     *
     ...
     *
     * <p>Example of compilation options:</p> <p>{@code Iterable&lt;String&gt; compilationOptions = Arrays.asList(new
     * String[]{"-source","1.6","-target","1 .6"});}</p>
     *
     * @param compilationOptions compilation options passed to the internal compiler. By default is null (default
     *                           compiler settings).
     *
     * @return this object for flow API use.
     */
     ...
     *
     * <p>If set to false other configuration parameters are ignored, there is no automatic source code change
     * detection/reload and original objects are returned instead of proxies, performance penalty is zero. Setting to
     * false is recommended in production whether source code change detection/reload is not required.</p>
     *
     ...
     *
     * <p>Structure of the source tree must be the same as a JavaSE application, the only difference is shell scripts,
     * shell scripts must be located on the top level of the source tree (default package).</p>
     *
     ...
     * System.err.println("  source: " + diagnostic.getCompleteSource());
     ...
     * </pre>
     *
     * @param diagnosticsListener the diagnostic listener to capture compilation errors and warnings. By default is
     *                            null, an internal listener is used logging to System.err.
     *
     * @return this object for flow API use.
     */
     ...
     *
     * <p>If this value is set to 0 or negative, no periodic source code change detection is executed and only
     * compilation on the fly happens in load time, this is valid for one shot scripts but it has no sense when using
     * proxies.
     *
