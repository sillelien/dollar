...
import me.neilellis.dollar.script.relproxy.RelProxy;
import me.neilellis.dollar.script.relproxy.RelProxyException;
import me.neilellis.dollar.script.relproxy.impl.jproxy.core.clsmgr.ClassDescriptorSourceScript;
import me.neilellis.dollar.script.relproxy.impl.jproxy.core.clsmgr.JProxyEngine;
import me.neilellis.dollar.script.relproxy.impl.jproxy.core.clsmgr.comp.JProxyCompilationException;
import me.neilellis.dollar.script.relproxy.impl.jproxy.shell.JProxyShellInteractiveImpl;
...
public class JProxyShellProcessor {
    ...
    private void execute(String code) {
        ...
        if (codeBufferModTimestamp > lastCodeCompiledTimestamp)  // Incluimos el = por si acaso va todo muy seguido
        {
            ...
            // Recuerda que cada vez que se obtiene el timestamp se llama a System.currentTimeMillis(), es imposible
            // que el usuario haga algo en menos de 1ms
            ...
            {
                // El ser humano es muy raro y es posible que a alguien se le ocurra usar el shell de forma
                // automatizada y se genere un siguiente cambio en el
                // código fuente tan rápido que no cambie el ms, así nos aseguramos con total rotundidad que la
                // modificación posterior de código fuente su timestamp es MAYOR que el de compilación último
                try {
                    Thread.sleep(1);
