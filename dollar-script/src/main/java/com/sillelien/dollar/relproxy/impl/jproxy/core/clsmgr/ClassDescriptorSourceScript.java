...
public class ClassDescriptorSourceScript extends ClassDescriptorSourceUnit {
    ...
    private void generateSourceCode() {
        ...
        finalCode.append("import com.sillelien.dollar.api.*;\n");
//        finalCode.append("import com.sillelien.dollar.script.lang.*;\n");
        finalCode.append("import static com.sillelien.dollar.api.DollarStatic.*;\n");
        ...
        if (completeClass) {
            if (hasHashBang[0]) {
                finalCode.append(
                        "\n");   // Como hemos quitado la línea #! añadimos una nueva para que los números de línea
            }
            // en caso de error coincidan con el original
            ...
            finalCode.append("public class " +
                             className +
                             " extends com.sillelien.dollar.api.Unit { public static " +
                             mainReturnType +
                             " main(" +
                             mainParamsDec +
                             ") {\n"); // Lo ponemos todo en una línea para que en caso de error la línea de error
            // coincida con el script original pues hemos quitado la primera línea #!
            finalCode.append(scriptCode);
            finalCode.append("  }\n");
            finalCode.append("}\n");
        }
    ...
    private boolean isCompleteClass(@NotNull String code) {
        // Buscamos si hay un " class ..." o un "import..." al comienzo para soportar la definición de una clase
        // completa como script
        int pos = code.indexOf("class");
        if (pos == -1) { return false; }
        // Hay al menos un "class", ojo que puede ser parte de una variable o dentro de un comentario, pero si no
        // existiera desde luego que no es clase completa
        ...
        visibility =
                visibility.trim(); // No consideramos \n hay que ser retorcido poner un \n entre el public y el class
        // por ejemplo
        if (visibility.isEmpty()) {
            return true; // No hay visibilidad, que no compile no es cosa nuestra
        }
    ...
    private int getFirstPosIgnoringCommentsAndSeparators(@NotNull String code) {
        ...
        for (i = 0; i < code.length(); i++) {
            ...
            if (c == ' ' || c == '\n' || c == '\t') { continue; } else if (c == '/' && i + 1 < code.length()) {
                char c2 = code.charAt(i + 1);
                if (c2 == '/') {
                    i = getFirstPosIgnoringOneLineComment(code, i);
                    if (i == -1) {
                        return -1; // Comentario mal formado
                    }
                } else if (c2 == '*') {
    ...
    public void callMainMethod(@NotNull LinkedList<String> argsToScript) throws Throwable {
        try {
            ...
            String[]
                    argsToScriptArr =
                    argsToScript.size() > 0 ? argsToScript.toArray(new String[argsToScript.size()]) : new String[0];
