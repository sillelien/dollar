...
import com.sillelien.dollar.relproxy.impl.jproxy.core.clsmgr.*;
...
public class JProxyCompilerInMemory {
    ...
    public JProxyCompilerInMemory(JProxyEngine engine, Iterable<String> compilationOptions,
    ...
    public void compileSourceFile(@NotNull ClassDescriptorSourceUnit sourceFileDesc,
        ...
        if (outClassList == null) { throw new JProxyCompilationException(sourceFileDesc); }

        String className = sourceFileDesc.getClassName();

        // Puede haber más de un resultado cuando hay inner classes y/o clase privada en el mismo archivo o bien
        // simplemente clases dependientes
        for (JavaFileObjectOutputClass outClass : outClassList) {
            ...
            if (className.equals(currClassName)) {
                ...
                if (innerClass != null) {
                    ...
                    // Lo mismo es un archivo dependiente e incluso una inner class pero de otra clase que está
                    // siendo usada en el archivo compilado
                    ...
                    if (dependentClass != null) {
                        ...
                        // Seguramente es debido a que el archivo java tiene una clase privada autónoma declarada en
                        // el mismo archivo .java, no permitimos estas clases porque sólo podemos
                        // detectarlas cuando cambiamos el código fuente, pero no si el código fuente no se ha
                        // tocado, por ejemplo no tenemos
                        // forma de conseguir que se recarguen de forma determinista y si posteriormente se cargara
                        // via ClassLoader al usarse no podemos reconocer que es una clase
                        // "hot reloadable" (quizás a través del package respecto a las demás clases hot pero no es
                        // muy determinista pues nada impide la mezcla de hot y no hot en el mismo package)
                        // Es una limitación mínima.
                        throw new RelProxyException("Unexpected class when compiling: " +
                                                    currClassName +
                                                    " maybe it is an autonomous private class declared in the same " +
                                                    "java file of the principal class, this kind of classes are not " +
                                                    "supported in hot reload");
                    }
                }
            }
        }
    ...
    private LinkedList<JavaFileObjectOutputClass> compile(ClassDescriptorSourceUnit sourceFileDesc,
        ...
        // .com/java/root/jdk/openjdk/6-b14/com/sun/tools/javac/util/JavacFileManager.java?av=h#JavacFileManager
        // http://grepcode.com/file/repository.grepcode.com/java/root/jdk/openjdk/7-b147/javax/tools/StandardLocation
        // .java
        ...
        // http://stackoverflow.com/questions/1563909/how-to-set-classpath-when-i-use-javax-tools-javacompiler
        // -compile-the-source
        // http://stackoverflow.com/questions/10767048/javacompiler-with-custom-classloader-and-filemanager

        StandardJavaFileManager
                standardFileManager =
                context.getStandardFileManager(); // recuerda que el StandardJavaFileManager puede reutilizarse entre
        // varias compilaciones consecutivas mientras se cierre al final
        ...
        JavaFileManagerInMemory
                fileManagerInMemory =
                new JavaFileManagerInMemory(standardFileManager, classLoader, sourceRegistry);
