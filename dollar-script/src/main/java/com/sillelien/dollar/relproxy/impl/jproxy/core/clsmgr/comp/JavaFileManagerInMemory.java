...
import com.sillelien.dollar.relproxy.impl.jproxy.core.clsmgr.ClassDescriptorSourceFileRegistry;
import com.sillelien.dollar.relproxy.impl.jproxy.core.clsmgr.ClassDescriptorSourceUnit;
...
public class JavaFileManagerInMemory extends ForwardingJavaFileManager {
    ...
    public Iterable list(Location location, @NotNull String packageName, @NotNull Set kinds, boolean recurse) throws
        ...
        {
            return super.list(location, packageName, kinds,
                              recurse);  // En este caso nunca (con PLATFORM_CLASS_PATH) va a encontrar nuestros
        }
        // sources ni .class
        else if (location == StandardLocation.CLASS_PATH && kinds.contains(Kind.CLASS)) {
            if (packageName.startsWith("java.") ||
                packageName.startsWith(
                        "javax."))  // a hack to let standard manager handle locations like "java.lang" or "java
            // .util". Estrictamente no es necesario pero derivamos la inmensa mayoría de las clases
            // estándar al método por defecto
            { return super.list(location, packageName, kinds, recurse); } else {
                // El StandardJavaFileManager al que hacemos forward es "configurado" por el compilador al que está
                // asociado cuando hay una tarea de compilación
                // dicha configuración es por ejemplo el classpath tanto para encontrar .class como .java
                // En nuestro caso no disponemos del classpath de los .class, disponemos del ClassLoader a través del
                // cual podemos obtener "a mano" via resources los
                // JavaFileObject de los .class que necesitamos.
                // Ahora bien, no es el caso de los archivos fuente en donde sí tenemos un path claro el cual pasamos
                // como classpath al compilador y por tanto un super.list(location, packageName, kinds, recurse)
                // nos devolverá los .java (como JavaFileObject claro) si encuentra archivos correspondientes al
                // package buscado.
                ...
                Iterable
                        inFileMgr =
                        super.list(location, packageName, kinds,
                                   recurse); // Esperamos o archivos fuente o .class de clases no recargables
                ...
                // Reemplazamos los .class de classList que son los que están en archivo "deployados" que pueden ser
                // más antiguos que los que están en memoria
                for (JavaFileObjectInputClassInFileSystem fileObj : classList) {
                    ...
                    if (sourceFileDesc != null && sourceFileDesc.getClassBytes() != null) {
                        JavaFileObjectInputClassInMemory
                                fileInput =
                                new JavaFileObjectInputClassInMemory(className, sourceFileDesc.getClassBytes(),
                                                                     sourceFileDesc.getTimestamp());
                ...
                // Los JavaFileObject de archivos fuente pueden ser los mimas clases que los de .class, el compilador
                // se encargará de comparar los timestamp y elegir el .class o el source
    ...
    public String inferBinaryName(Location location, JavaFileObject file) {
        if (file instanceof JProxyJavaFileObjectInput) { return ((JProxyJavaFileObjectInput) file).getBinaryName(); }
