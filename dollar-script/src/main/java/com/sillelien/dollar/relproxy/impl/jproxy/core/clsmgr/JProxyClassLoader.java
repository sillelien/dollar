...
public class JProxyClassLoader extends ClassLoader {
    ...
    public Class loadClass(@NotNull ClassDescriptor classDesc, boolean resolve) {
        synchronized (engine) {
            ...
            if (clasz != null && clasz.getClassLoader() == this) {
                return clasz; // Glup, ya fue cargada
            }
    ...
    protected Class<?> loadClass(@NotNull String name, boolean resolve) throws ClassNotFoundException {
        ...
        // Lo redefinimos por si acaso porque el objetivo es recargar todas las clases hot-reloaded en este
        // ClassLoader y no delegar en el parent
        // (el comportamiento por defecto de loadClass) pues las clases cargadas con el parent tenderán a cargar las
        // clases vinculadas con dicho ClassLoader

        // En teoría este método redefinido no es necesario porque manualmente detectamos los cambios de código
        // fuente, recompilamos y recargamos explícitamente
        // con defineClass el cual no carga también las innerclasses vinculadas, 
        // pero si el código fuente tiene innerclasses y no ha sido cambiado nunca, las innerclasses pueden no ser
        // conocidas como ClassDescriptor,
        // necesitamos detectar las innerclasses para cargarlas también tras la carga de la clase contenedora,
        // para ello ejecutamos Class.getDeclaredClasses() para que cargue las innerclasses indirectamente, pasando
        // entonces por aquí.

        // Hay un caso más y es el de la clase base que es una clase hot loadable con su propio archivo, al ejecutar
        // el defineClass en la clase
        // derivada la clase base también debe cargarse en ese momento y es posible que no haya sido hecho
        // explícitamente (por ej porque no ha cambiado o su carga va después)
        // por lo que pasaremos por aquí y debemos cargarla aquí, luego no hay problema de recarga explícita porque
        // sabemos que ha sido ya cargada
        // y tampoco hay problema de auto-salvado del .class o eliminación del mismo puesto que al ser un archivo
        // fuente normal se tratará por si mismo
        ...
        synchronized (engine) {
            ...
            if (cls == null) {
                ClassDescriptor
                        classDesc =
                        engine.getClassDescriptor(
                                name); // Si es una inner class se crea el descriptor y se añade al source file
