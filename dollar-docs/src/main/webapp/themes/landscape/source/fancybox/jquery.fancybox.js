		...
		function (window, document, $, undefined) {
			...
			$.extend(F, {
				...
				play: function (action) {
					...
					if (action === true || (
							!F.player.isActive && action !== false
							)) {
						start();
					} else {
						stop();
				...
				jumpto: function (index, direction, router) {
					...
					F.direction = direction || current.direction[(
				...
				update: function (e) {
					var type = (
							e && e.originalEvent && e.originalEvent.type
							),
				...
				_start: function (index) {
					...
					) + ' fancybox-type-' + type + ' fancybox-tmp ' + coming.wrapCSS).appendTo(coming.parent || 'body');
				...
				_getPosition: function (onlyAbsolute) {
					...
					rez.top = getValue(Math.max(rez.top,
												rez.top + (
												(
												viewport.h - height
												) * current.topRatio
												)));
					rez.left = getValue(Math.max(rez.left,
												 rez.left + (
												 (
												 viewport.w - width
												 ) * current.leftRatio
				...
				beforeShow: function (opts, obj) {
					if (obj.locked && !this.el.hasClass('fancybox-lock')) {
						if (this.fixPosition !== false) {
							$('*').filter(function () {
								return (
								$(this).css('position') === 'fixed' && !$(this).hasClass("fancybox-overlay") && !$(this).hasClass("fancybox-wrap")
								);
			...
			F.helpers.title = {
				...
				beforeShow: function (opts) {
					...
					title[(
							opts.position === 'top' ? 'prependTo' : 'appendTo'
					)](target);
				}
			};
			...
			$.fn.fancybox = function (options) {
				...
				if (!selector || options.live === false) {
					...
					D.undelegate(selector,
								 'click.fb-start').delegate(selector + ":not('.fancybox-item, .fancybox-nav')",
															'click.fb-start',
															run);
				}
